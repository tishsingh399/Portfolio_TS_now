...
// Source: github.com/DavidHDev/react-bits — Backgrounds/PixelBlast
// Converted to TypeScript for Next.js
...
const createLiquidEffect = (texture: THREE.Texture, opts?: { strength?: number; freq?: number }) => {
  ...
  `;
  const uniformMap = new Map<string, THREE.Uniform<unknown>>();
  uniformMap.set('uTexture', new THREE.Uniform(texture));
  uniformMap.set('uStrength', new THREE.Uniform(opts?.strength ?? 0.025));
  uniformMap.set('uTime', new THREE.Uniform(0));
  uniformMap.set('uFreq', new THREE.Uniform(opts?.freq ?? 4.5));
  return new Effect('LiquidEffect', fragment, { uniforms: uniformMap });
};
...
void main(){
  ...
  const float dampT=1.0, dampR=10.0;
  if(uEnableRipples==1){
...
export default function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const timeOffset = (crypto?.getRandomValues ? (() => { const u32 = new Uint32Array(1); crypto.getRandomValues(u32); return u32[0] / 0xffffffff; })() : Math.random()) * 1000;
      ...
      const animate = () => {
        ...
        if (liquidEffect) (liquidEffect.uniforms as Map<string, THREE.Uniform<any>>).get('uTime')!.value = uniforms.uTime.value;
        if (composer) {
